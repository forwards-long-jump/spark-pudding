...
function update()
  ...
  for i, entity in ipairs(selectedEntities) do
    ...
    for i = 0, 8 do
      ...
      if game.input:isMouseInRectangle(x, y, cursorSize, cursorSize) then
        ...
        if draggingAction == -1 and game.input:isMouseButtonDown(1) then
          ...
          draggingStartEntityPosition.x = entity.position.x
          draggingStartEntityPosition.y = entity.position.y
        end
        break
      end
    end
    ...
    if draggingAction == 4 then
      -- Move entity
      entity.position.x = game.input:getMouseX() - (entity.size.width / 2)
      entity.position.y = game.input:getMouseY() - (entity.size.height / 2)
    end
    ...
    if draggingAction == 6 then
      ...
      entity.size.height = -entity.position.y + game.input:getMouseY()
    end
